...
AFRAME.registerComponent('resonance-audio-room', {
  ...
  tick () {
	    ...
	    this.cameraMatrix4 = cameraEl.object3D.matrixWorld
			...
			this.source.setFromMatrix(this.sound.getMatrixWorld())
		}
  },
  ...
  setUpAudio () {
    ...
    // Play the audio.
		if (this.resonanceAudioContext.state === "running" && this.sound.data.autoplay) {
    ...
    this.el.audioElement.setAttribute('loop', this.sound.data.loop)
		...
		source.setDirectivityPattern(this.sound.data.alpha, this.sound.data.sharpness)

		// Set gainNode
		source.setGain(this.sound.data.gain)

		// Set max distance
		source.setMaxDistance(this.sound.data.maxDistance)
  },

  remove () {
    this.el.audioElement.pause()
    this.el.audioElement = null
  },
