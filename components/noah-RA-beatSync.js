AFRAME.registerComponent('beat-sync', {
  ...
  schema: {
    ...
    analyzeStereo: {type: 'boolean', default: false}
  },
  ...
  getBeats() {
    ...
    fetch(this.audioEl.getAttribute('src'), {mode: "cors"}).then(resp => {return resp.arrayBuffer()})
    .then(buffer => {
      ...
      const offline = new OfflineAudioContext(1, buffer.byteLength, 44100);
      // Decode the audio file, then pass to worker to process beat data
      offline.decodeAudioData(buffer, decBuffer =>{
        worker.postMessage({buffer: decBuffer.getChannelData(0)})
        worker.onmessage = e => {
          this.audioEl.beatData = e.data.beatData
  ...
  emitEvent () {
    if (this.audioEl) {
      if (this.audioContext.state === "running" && this.audioEl.beatData && this.audioEl.beatData !== "loading") {
        ...
        if (currentTime < this.audioEl.beatData.beats[0]) {
          this.beatIdx = 0
        }
        //correct course if current song time gets ahead of beat index
        else if (currentTime > this.audioEl.beatData.beats[this.beatIdx]) {
          const findBeat = (beat, i) => {
            return (beat-currentTime < beat - this.audioEl.beatData.beats[i-1] && beat-currentTime > 0)
          }
          this.beatIdx = this.audioEl.beatData.beats.findIndex(findBeat)
        }
        ...
        if (this.audioEl.beatData.beats[this.beatIdx] - currentTime < this.data.threshold) {
          this.beatInc()
        }
      }
    }
  ...
  beatInc () {
    ...
    if (this.data.frequency < 1) {
      ...
      const curBeat = this.audioEl.beatData.beats[this.beatIdx]
      const prevBeat = this.audioEl.beatData.beats[this.beatIdx - 1]
      const interval = Math.abs(curBeat - prevBeat)
      const beatArray = []
      for (var i=0; i < Math.floor(1/this.data.frequency); i++){
...
const workerFn = function () {
  this.onmessage = function (e) {
    this.postMessage({beatData: new MusicTempo(e.data.buffer)})
  }
